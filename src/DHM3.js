...
class DHM extends Phaser.Scene{ //¿Qué número es?
    ...
    create(){
        var image = this.add.image(170,120,'logo').setScale(0.5);
        //image.displayWidth = 400;
        ...
        this.add.text(960, 200-35, '¿Qué hay más?', { color: 'black', fontFamily: 'Arial', fontSize: '70px '}).setOrigin(0.5,0);
        var boton = document.createElement('button');
        boton.setAttribute('type','button');
        this.element = this.add.dom(700, 300, boton,'','Rojo');
        this.element.addListener('click');
        this.element.on('click',() => this.corregir(1));
        
        var boton = document.createElement('button');
        boton.setAttribute('type','button');
        this.element = this.add.dom(1000, 300, boton,'','Azul');
        this.element.addListener('click');
        this.element.on('click',() => this.corregir(2));
        
        var boton = document.createElement('button');
        boton.setAttribute('type','button');
        this.element = this.add.dom(1300, 300, boton,'','Iguales');
        this.element.addListener('click');
        this.element.on('click',() => this.corregir(0));
        ...
        for(var j=0;j<2;j++){
            for(var i=0;i<this.N[j];i++){
                this.unos[j].push(this.add.image(710+500*j + this.r*Math.cos(2*Math.PI/this.N[j]*i),590+ this.r*Math.sin(2*Math.PI/this.N[j]*i),'p').setTint((0xff0000)*(1-j)+(0x0000ff)*j));
    ...
    corregir(r){
        ...
        if(r === this.resp){
            ...
            this.lienzo3 = this.add.graphics();
                ...
                const otro = this.add.zone(1600,50,300,90);
                    otro.setOrigin(0);
                    otro.setInteractive();
                    otro.on('pointerdown', () => this.opcionPulsada('otro'));
            a=0;
...
class DHM2 extends Phaser.Scene{ //¿Qué número es?
    ...
    create(){
        ...
        for(var j=0;j<2;j++){
            for(var i=0;i<this.N[j];i++){
                this.unos[j].push(this.add.image(960-2*this.r+4*this.r*j + this.r*Math.cos(2*Math.PI/this.N[j]*i),590+ this.r*Math.sin(2*Math.PI/this.N[j]*i),'p').setTint((0xff0000)*(1-j)+(0x0000ff)*j));
                this.unos[j][i].setInteractive();
                this.input.setDraggable(this.unos[j][i]);
                this.input.on('drag', function (pointer, gameObject, dragX, dragY) {
                    gameObject.x = dragX;
        ...
        if(rc === 1){ //Calcula velocidades
            ...
            for(var i=0;i<this.unos.length;i++){
                for(var j=0;j<this.unos[i].length;j++){
                    this.vel[i].push([210+j*100-this.unos[i][j].x,400+i*100-this.unos[i][j].y]);
                }
            }
            //console.log(this.vel);
            rc++;
...
class DHM3 extends Phaser.Scene{ //¿Qué número es?
    ...
    create(){
        this.add.image(170,120,'logo').setScale(0.5);
        ...
        this.add.text(960, 100-35, '¿Qué hay más?', { color: 'black', fontFamily: 'Arial', fontSize: '70px '}).setOrigin(0.5,0);
        
        
        var boton = document.createElement('button');
        boton.setAttribute('type','button');
        this.element = this.add.dom(600, 200, boton,'','Rojo');
        this.element.addListener('click');
        this.element.on('click',() => this.corregir(1));
        
        var boton = document.createElement('button');
        boton.setAttribute('type','button');
        this.element = this.add.dom(900, 200, boton,'','Azul');
        this.element.addListener('click');
        this.element.on('click',() => this.corregir(2));
        
        var boton = document.createElement('button');
        boton.setAttribute('type','button');
        this.element = this.add.dom(1200, 200, boton,'','Verde');
        this.element.addListener('click');
        this.element.on('click',() => this.corregir(3));
        
        var boton = document.createElement('button');
        boton.setAttribute('type','button');
        this.element = this.add.dom(1500, 200, boton,'','Iguales');
        this.element.addListener('click');
        this.element.on('click',() => this.corregir(0));
